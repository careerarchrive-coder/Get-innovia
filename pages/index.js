...
const hasActiveFilters = Boolean(keyword || location || classification || jobType);
...
{locationSuggestions.map((loc) => (
      <li key={loc} onMouseDown={() => selectLocationSuggestion(loc)}>{loc}</li>
                         ))}
</ul>
)}
</div>
...
background: #fff; border: 1px solid #e2e2f0; border-radius: 10px;
box-shadow: 0 12px 28px -8px rgba(0,0,0,0.2); list-style: none;
...
.suggestions li:hover { background: #f1edff; color: #5b21b6; }
.clear-btn {
...
.job-title-link:hover .job-title { color: #6d28d9; }
.job-meta-line { color: #5b5b78; font-size: 0.9rem; margin-bottom: 0.75rem; }
...
export async function getStaticProps() {
      try {
            ...
            return { props: { jobs: [], error: true }, revalidate: 60 };
      }
}
